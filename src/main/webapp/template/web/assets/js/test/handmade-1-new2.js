			...
			if (slide_t == "r")
			{
				if (f03sliderIdx == sliderSize)
				{
					...
					setTimeout(function(){
						...
						for (var i = 0;i <= sliderFull ; i++ )
						{
							if (i < slide_idx + 2)
							{
								...
								f03Slider.eq(i).stop().css({"margin-left":-(movePosition * 75) + "%"});
							}else if (i > slide_idx + 2)
							{
								var movePosition = i - slide_idx - 2;
								f03Slider.eq(i).stop().css({"margin-left": (movePosition * 92) + "%"});
							}else{
					...
					for (var i = 0;i <= sliderFull ; i++ )
					{
						...
						{
							var movePosition = slide_idx + 2 - i;
							f03Slider.eq(i).stop().animate({"margin-left":-(movePosition * 75) + "%"},motionDelay);
						}else if (i > slide_idx + 2)
						{
							var movePosition = i - slide_idx - 2;
							f03Slider.eq(i).stop().animate({"margin-left": (movePosition * 92) + "%"},motionDelay);
						}else{
				...
				// Sửa khoảng cách giữa các ảnh thong slide videos
				if (f03sliderIdx == 0)
				{
					...
					f03Slider.eq(2).stop().animate({"margin-left": "92%"},motionDelay, function(){
						f03Slider.eq(sliderFull-1).css({"margin-left":"92%"});
						$(this).css({"margin-left": ((sliderSize-1) * 100) + "%"});
					});
					f03Slider.eq(3).stop().animate({"margin-left": "184%"},motionDelay, function(){
						f03Slider.eq(sliderFull).css({"margin-left":"184%"});
						$(this).css({"margin-left": ((sliderSize-2) * 100) + "%"});
					});
